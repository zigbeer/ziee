    ...
    switch (this.sid) {
        ...
        case 'cmds':
            _.forEach(resrcs, function (cmdCb, cmdId) {
                ...
                if (zcl) {
                    if (!zclId.functional(self.cid, cmdId) && !zclId.getCmdRsp(self.cid, cmdId))
                        throw new TypeError('Command id: ' + cmdId + ' is not an ZCL-defined functional command or command response.');
                }
